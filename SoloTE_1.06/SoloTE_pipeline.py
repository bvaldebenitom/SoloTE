...
def processPerChrom(chromosome,inputfile,outdir,outbasename):
    ...
    cmd="samtools view "+inputfile+" "+chromosome+"| awk 'BEGIN{FS=OFS=\"\t\"}{gn=\"\";gx=\"\";cb=\"\";ub=\"\";for(i=12;i<=NF;i++){if($i~ /^CB:Z:/){cb=$i;sub(\"CB:Z:\",\"\",cb)};if($i~ /^UB:Z:/){ub=$i;sub(\"UB:Z:\",\"\",ub)};if($i~ /^GX:Z:/){gx=$i};if($i~ /^GN:Z:/){gn=$i;sub(\"GN:Z:\",\"\",gn)} }; if(cb!=\"-\" && ub != \"-\" && $0 ~ /CB:/ && $0 ~/UB:/){print gn,cb,ub}}'|sort -k1,1 -k2,2 -k3,3|bedtools groupby -g 1,2 -c 3 -o count_distinct >  "+countsname
    print(cmd)
    os.system(cmd)
